...
async function testServers(server) {
  ...
  try {
    ...
    logError(`测试服务器 ${server.name || server.url} 失败:`, error.message);
...
async function performUploadTest(client) {
  try {
    ...
    logError('上传测试失败:', error.message);
    
    // 尝试使用较小的数据包进行测试
    try {
      logInfo('尝试使用较小的数据包进行上传测试...');
      ...
      logError('小数据包上传测试也失败:', smallError.message);
...
async function performPingTest(client) {
  try {
    ...
    logError('ping测试失败:', error.message);
    throw error;
  }
}

...
async function verifyResults() {
  ...
  try {
    ...
    results.forEach((result, index) => {
      ...
      logInfo(`  服务器名称: ${result.server_name}`);
      logInfo(`  服务器URL: ${result.server_url}`);
      logInfo(`  测试时间: ${result.test_timestamp}`);
      logInfo(`  下载速度: ${result.download_speed !== null && result.download_speed !== undefined ? result.download_speed + ' Mbps' : 'N/A'}`);
      logInfo(`  上传速度: ${result.upload_speed !== null && result.upload_speed !== undefined ? result.upload_speed + ' Mbps' : 'N/A'}`);
      logInfo(`  延迟: ${result.ping !== null && result.ping !== undefined ? result.ping + ' ms' : 'N/A'}`);
      logInfo(`  抖动: ${result.jitter !== null && result.jitter !== undefined ? result.jitter + ' ms' : 'N/A'}`);
      logInfo(`  服务器信息: ${JSON.stringify(result.server_info, null, 2)}`);
      logInfo('----------------------------------------');
    });
    ...
    results.forEach((result, index) => {
      ...
      if (!result.server_name || !result.server_url) {
        logInfo(`❌ 记录 ${recordNumber}: 缺少服务器名称或URL`);
        allValid = false;
      }
      ...
      if (result.download_speed !== null && result.download_speed !== undefined) {
        if (typeof result.download_speed !== 'number' || isNaN(result.download_speed) || result.download_speed < 0) {
          logInfo(`❌ 记录 ${recordNumber}: 下载速度无效`);
          allValid = false;
        }
      }
      ...
      if (result.upload_speed !== null && result.upload_speed !== undefined) {
        if (typeof result.upload_speed !== 'number' || isNaN(result.upload_speed) || result.upload_speed < 0) {
          logInfo(`❌ 记录 ${recordNumber}: 上传速度无效`);
          allValid = false;
        }
      }
      ...
      if (!result.test_timestamp) {
        logInfo(`❌ 记录 ${recordNumber}: 缺少测试时间戳`);
        allValid = false;
      }
    });
    
    if (allValid) {
      logInfo('✅ 所有记录的数据完整性验证通过');
    } else {
